...
const RankingItem = ({
  ...
  useEffect(() => {
    ...
    if (tokenObj?.price) {
      ...
      setTimeout(async () => await fetchPrice(collectionID, token_id), delay);
    }
  ...
  const fetchPrice = async (collectionID, token_id) => {
    await axios
      .request(
        `https://api.opensea.io/api/v1/asset/${collectionID}/${token_id}/`,
        {
          headers: {
            accept: '*/*',
          },
